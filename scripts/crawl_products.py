...
def main():
    if len(sys.argv) < 2:
        print("Usage: python scripts/crawl_products.py <store_id>")
        print("  Crawl products for a store from 1688 and/or AliExpress.")
        sys.exit(1)

    store_id = sys.argv[1]
    cfg_path = Path("config/stores") / f"{store_id}.json"
    if not cfg_path.exists():
        logger.error(f"Store config not found: {cfg_path}")
        return
    ...
    logger.info(f"Saved {len(out)} products to {out_path}")
